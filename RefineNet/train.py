...
def main(args):
    ...
    cs_path = '/tudelft.net/staff-umbrella/StudentsCVlab/nju/real-semantic-segmentation/minicity'
    # cs_path = '/tudelft.net/staff-umbrella/StudentsCVlab/nju/final/CIConv-semantic-segmentation/dataset/cityscapesresized'
    # cs_path = '/tudelft.net/staff-umbrella/StudentsCVlab/nju/final/CIConv-semantic-segmentation/dataset/Cityscapes'

    nd_path = '/tudelft.net/staff-bulk/ewi/insy/CV-DataSets/NighttimeDrivingTest/'
    # nd_path = '/tudelft.net/staff-umbrella/StudentsCVlab/nju/final/CIConv-semantic-segmentation/dataset/nighttimedrivingresized'
    dz_path = '/tudelft.net/staff-bulk/ewi/insy/CV-DataSets/Dark_Zurich_val_anon/'
    # dz_path = '/tudelft.net/staff-umbrella/StudentsCVlab/nju/final/CIConv-semantic-segmentation/dataset/Dark_Zurich_val_anonresized'
    ...
    # Load dataset
    trainset = MiniCity(cs_path, split='train', transforms=train_trans)
    valset = MiniCity(cs_path, split='val', transforms=test_trans)
    testset_day = MiniCity(cs_path, split='test', transforms=test_trans)
    ...
    # model = Deeplab(num_classes, invariant=args.invariant, k=3, scale=0.0)
    # model = RefineNet()
    print(model)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        with open('logs/log_epoch.csv', 'a') as epoch_log:
            ...
            epoch_log.write('{}, {:.5f}, {:.5f}, {:.5f}, {:.5f}, {:.5f}, {:.5f}, {:.5f}, {:.5f}, {:.5f}\n'.format(
                    epoch, train_loss, val_loss_cs, train_acc, val_acc_cs, test_acc_nd, miou_cs, miou_nd, miou_dz, lr))
    ...
    #CHANGED
    print('Best mIoU: {:4f}, corresponding val acc: NOT APPLICABLE?'.format(best_miou))
    ...
    #CHANGED
    for i in range(len(metrics['val_acc_cs'])):
        metrics['val_acc_cs'][i] = metrics['val_acc_cs'][i].cpu();
    for i in range(len(metrics['test_acc_nd'])):
        metrics['test_acc_nd'][i] = metrics['test_acc_nd'][i].cpu();
    for i in range(len(metrics['test_acc_dz'])):
        metrics['test_acc_dz'][i] = metrics['test_acc_dz'][i].cpu();
